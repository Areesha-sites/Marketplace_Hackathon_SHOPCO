...
import { NewArrivalDetailsPropsTypes } from "../../../../types/ComponentsTypes";
import {
  Sheet,
  SheetClose,
  SheetContent,
...
interface CartItem extends NewArrivalDetailsPropsTypes {
  quantity: number;
  index: number; 
}
const NewArrivalDetails: React.FC<Props> = ({ params }) => {
  ...
  const [isActive, setIsActive] = useState<string>("large");
  ...
  const addToCart = (product: NewArrivalDetailsPropsTypes) => {
    ...
    if (productInCart) {
      ...
      const updatedCart = [
        ...cart,
        { ...product, quantity: 1, index: cart.length },
      ];
      setCart(updatedCart);
      localStorage.setItem("cart", JSON.stringify(updatedCart));
    }
  ...
  useEffect(() => {
    ...
    if (savedCart) {
      const parsedCart = JSON.parse(savedCart) as CartItem[]; // Type assertion
      setCart(parsedCart.map((item, index) => ({ ...item, index }))); // Add index to each item
    }
  }, []);
  ...
  const { id } = params;
  const product: NewArrivalDetailsPropsTypes | undefined = newArrivalCardsData.find(
    (item: NewArrivalDetailsPropsTypes) => item.id === id
  );
  if (!product) {
    return (
    ...
    <>
      <section className="">
        ...
        <Image
          ...
          height={167}
          width={152}
          className="md:h-[167px] md:w-[152px] absolute md:top-[397px] md:left-[100px] rounded-[20px] w-[112px] h-[106px] top-[475px] left-[139px] -hue-rotate-30"
        />
        <Image
          data-aos="flip-up"
          data-aos-delay="400"
