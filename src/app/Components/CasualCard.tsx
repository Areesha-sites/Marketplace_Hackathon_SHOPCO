...
const CasualCards = ({id, image, title, price, rating, ratingReview, discount, offer, positionStyles}:CasualCardsPropsTypes) => {
  return (
    <>
     <Link href={`/newArrival/${id}`} passHref>
      <div className="w-[295px] h-[298px] rounded-[20px] bg-BannerBgColor "
      style={{ position: "absolute", ...positionStyles }}>
        ...
        <p className="text-[24px] font-satoshi font-bold absolute  top-[374px] left-[3px] text-black">${price}</p>
        <p className="text-[24px] font-satoshi font-bold absolute  top-[374px] left-[70px] text-black/40 line-through">${price}</p>
        <span className="w-[58px] h-[28px] py-[6px] px-[14px] rounded-[62px] bg-redOfferBgColor bg-opacity-10 text-[12px] font-medium text-redTextOfferColor absolute  top-[378px] left-[140px]">
            {offer}
          </span>
